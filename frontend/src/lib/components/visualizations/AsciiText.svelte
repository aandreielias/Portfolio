<script>
  ...
  // The JSX had this assigned to Math.map, I'll keep it as a const for safety in module scope
  ...
  $: if (asciiInstance && text) {
    asciiInstance.updateText(text);
  }
</script>
...
<style>
  ...
  :global(.ascii-text-container pre) {
    ...
    left: 0;
    top: 0;
    /* Using Black/Zinc gradient as per user requirement, adapting from 'as is' logic */
